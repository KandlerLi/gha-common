...
Callers pin a specific semver tag (`@v0.0.5`), never `@main` — matches this
...
Then bump the `@v0.0.5` → `@v0.0.6` reference in whichever caller repo(s)
...
jobs:
  terraform:
    uses: KandlerLi/gha-common/.github/workflows/terraform-checks.yml@v0.0.5
    with:
...
jobs:
  terraform:
    uses: KandlerLi/gha-common/.github/workflows/terraform-apply.yml@v0.0.5
    with:
      aws_region: eu-central-1
      extra_repo_vars: "ROUTE53_ZONE_ID"
    secrets: inherit
